...
const envSetCmd = new Command("set").usage("[options] <name> <value>").argument("[name]", "The name of the environment variable to set.").argument(
  "[value]",
...
export async function selectEnvDeployment(options) {
...
const envGetCmd = new Command("get").argument("<name>", "The name of the environment variable to print.").summary("Print a variable's value").description("Print a variable's value: `npx convex env get NAME`").configureHelp({ showGlobalOptions: true }).allowExcessArguments(false).action(async (envVarName, _options, cmd) => {
  const options = cmd.optsWithGlobals();
  const { ctx, deployment } = await selectEnvDeployment(options);
...
const envRemoveCmd = new Command("remove").alias("rm").alias("unset").argument("<name>", "The name of the environment variable to unset.").summary("Unset a variable").description(
...
const envListCmd = new Command("list").summary("List all variables and their values").description(
  [
...
).addCommand(envSetCmd).addCommand(envGetCmd).addCommand(envRemoveCmd).addCommand(envListCmd).addCommand(envDefault).helpCommand(false).addDeploymentSelectionOptions(
